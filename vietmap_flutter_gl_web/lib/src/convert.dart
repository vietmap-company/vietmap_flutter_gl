...
class Convert {
  static void interpretMapLibreMapOptions(
      Map<String, dynamic> options, VietmapMapOptionsSink sink) {
    ...
    if (options.containsKey('compassViewMargins')) {
      sink.setCompassViewMargins(options['compassViewMargins'][0].round(),
          options['compassViewMargins'][1].round());
    }
  ...
  static CameraOptions toCameraOptions(
      CameraUpdate cameraUpdate, VietmapGL mapLibreMap) {
  ...
  static Feature interpretLineOptions(
  ...
  static Feature intepretFillOptions(PolygonOptions options, Feature feature) {
    final properties = feature.properties;
    var geometry = feature.geometry;
    if (options.draggable != null) {
      properties['draggable'] = options.draggable;
