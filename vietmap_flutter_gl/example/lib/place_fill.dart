...
class PlaceFillBodyState extends State<PlaceFillBody> {
  ...
  void _updateSelectedFill(PolygonOptions changes) {
    controller!.updatePolygon(_selectedFill!, changes);
  }
  ...
  void _remove() {
    controller!.removePolygon(_selectedFill!);
    setState(() {
      _selectedFill = null;
      _fillCount -= 1;
    });
  ...
  void _changePosition() {
    ...
    _updateSelectedFill(PolygonOptions(
        geometry: geometry
            .map((list) => list
                .map(
