...
typedef MaplibreMapController = VietmapController;
...
///
/// To add annotations ([Circle]s, [Line]s, [Symbol]s and [Polygon]s) on the map, there are two ways:
///
/// 1. *Simple way to add annotations*: Use the corresponding add* methods ([addCircle], [addPolyline], [addSymbol] and [addPolygon]) on the MapLibreMapController to add one annotation at a time to the map.
/// There are also corresponding [addCircles], [addPolylines] etc. methods which work the same but add multiple annotations at a time.
///
...
/// (i.e. the collection of [Symbol]s, [Line]s, [Circle]s and [Polygon]s added to this map via the "simple way" (see above))
...
class VietmapController extends ChangeNotifier {
  ...
  Future<void> addFillExtrusionLayer(
      ...
      dynamic filter,
      bool enableInteraction = true}) async {
    await _vietmapGLPlatform.addFillExtrusionLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
  ...
  Future<void> addHeatmapLayer(
      ...
      double? minzoom,
      double? maxzoom}) async {
    await _vietmapGLPlatform.addHeatmapLayer(
      sourceId,
      layerId,
      properties.toJson(),
      belowLayerId: belowLayerId,
