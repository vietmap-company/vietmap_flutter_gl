...
class VietmapSphericalUtil {
  ...
  static num computeHeading(LatLng from, LatLng to) {
    ...
    final fromLat = VietmapMathUtil.toRadians(from.latitude);
    final fromLng = VietmapMathUtil.toRadians(from.longitude);
    final toLat = VietmapMathUtil.toRadians(to.latitude);
    final toLng = VietmapMathUtil.toRadians(to.longitude);
    final dLng = toLng - fromLng;
    final heading = atan2(sin(dLng) * cos(toLat),
        cos(fromLat) * sin(toLat) - sin(fromLat) * cos(toLat) * cos(dLng));

    return VietmapMathUtil.wrap(VietmapMathUtil.toDegrees(heading), -180, 180);
  }
  ...
  static LatLng computeOffset(LatLng from, num distance, num heading) {
    ...
    heading = VietmapMathUtil.toRadians(heading);
    ...
    return LatLng(VietmapMathUtil.toDegrees(asin(sinLat)).toDouble(),
        VietmapMathUtil.toDegrees(fromLng + dLng).toDouble());
  }
  ...
  static LatLng? computeOffsetOrigin(LatLng to, num distance, num heading) {
    ...
    final n4 = sin(VietmapMathUtil.toRadians(to.latitude));
    ...
    final fromLngRadians = VietmapMathUtil.toRadians(to.longitude) -
        atan2(n3, n1 * cos(fromLatRadians) - n2 * sin(fromLatRadians));
    return LatLng(VietmapMathUtil.toDegrees(fromLatRadians).toDouble(),
        VietmapMathUtil.toDegrees(fromLngRadians).toDouble());
  }
  ...
  static LatLng interpolate(LatLng from, LatLng to, num fraction) {
    ...
    return LatLng(VietmapMathUtil.toDegrees(lat).toDouble(),
        VietmapMathUtil.toDegrees(lng).toDouble());
  }

  /// Returns distance on the unit sphere; the arguments are in radians.
  static num distanceRadians(num lat1, num lng1, num lat2, num lng2) =>
      VietmapMathUtil.arcHav(
          VietmapMathUtil.havDistance(lat1, lat2, lng1 - lng2));
  ...
  static num computeAngleBetween(LatLng from, LatLng to) => distanceRadians(
      VietmapMathUtil.toRadians(from.latitude),
      VietmapMathUtil.toRadians(from.longitude),
      VietmapMathUtil.toRadians(to.latitude),
      VietmapMathUtil.toRadians(to.longitude));
  ...
  static num computeLength(List<LatLng> path) {
    ...
    var prevLat = VietmapMathUtil.toRadians(prev.latitude);
    var prevLng = VietmapMathUtil.toRadians(prev.longitude);

    final length = path.fold<num>(0.0, (value, point) {
      final lat = VietmapMathUtil.toRadians(point.latitude);
      final lng = VietmapMathUtil.toRadians(point.longitude);
  ...
  static num _computeSignedArea(List<LatLng> path, num radius) {
    ...
    var prevTanLat =
        tan((pi / 2 - VietmapMathUtil.toRadians(prev.latitude)) / 2);
    var prevLng = VietmapMathUtil.toRadians(prev.longitude);
    ...
    final total = path.fold<num>(0.0, (value, point) {
      final tanLat =
          tan((pi / 2 - VietmapMathUtil.toRadians(point.latitude)) / 2);
      final lng = VietmapMathUtil.toRadians(point.longitude);
