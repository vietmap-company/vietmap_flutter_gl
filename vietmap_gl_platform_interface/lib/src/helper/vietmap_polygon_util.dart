...
class VietmapPolygonUtil {
  ...
  static num _mercatorLatRhumb(num lat1, num lat2, num lng2, num lng3) =>
      (VietmapMathUtil.mercator(lat1) * (lng2 - lng3) +
          VietmapMathUtil.mercator(lat2) * lng3) /
      lng2;
  ...
  static bool _intersects(
    ...
    return geodesic
        ...
        : VietmapMathUtil.mercator(lat3) >=
            _mercatorLatRhumb(lat1, lat2, lng2, lng3);
  }
  ...
  static bool containsLocationAtLatLng(
    ...
    final lat3 = VietmapMathUtil.toRadians(latitude);
    final lng3 = VietmapMathUtil.toRadians(longitude);
    final prev = polygon.last;
    var lat1 = VietmapMathUtil.toRadians(prev.latitude);
    var lng1 = VietmapMathUtil.toRadians(prev.longitude);
    var nIntersect = 0;

    for (final point2 in polygon) {
      final dLng3 = VietmapMathUtil.wrap(lng3 - lng1, -pi, pi);
      // Special case: point equal to vertex is inside.
      if (lat3 == lat1 && dLng3 == 0) {
        return true;
      }
      final lat2 = VietmapMathUtil.toRadians(point2.latitude);
      final lng2 = VietmapMathUtil.toRadians(point2.longitude);
      // Offset longitudes by -lng1.
      if (_intersects(lat1, lat2, VietmapMathUtil.wrap(lng2 - lng1, -pi, pi),
          lat3, dLng3, geodesic)) {
        ++nIntersect;
      }
      lat1 = lat2;
  ...
  static int locationIndexOnEdgeOrPath(LatLng point, List<LatLng> poly,
    ...
    final havTolerance = VietmapMathUtil.hav(tolerance);
    final lat3 = VietmapMathUtil.toRadians(point.latitude);
    final lng3 = VietmapMathUtil.toRadians(point.longitude);
    ...
    if (geodesic) {
      ...
      var y1 = VietmapMathUtil.mercator(lat1);
      final y3 = VietmapMathUtil.mercator(lat3);
      final xTry = List<num?>.generate(3, (index) => null);

      for (final point2 in poly) {
        final lat2 = VietmapMathUtil.toRadians(point2.latitude);
        final y2 = VietmapMathUtil.mercator(lat2);
        final lng2 = VietmapMathUtil.toRadians(point2.longitude);
        if (max(lat1, lat2) >= minAcceptable &&
            min(lat1, lat2) <= maxAcceptable) {
          // We offset longitudes by -lng1; the implicit x1 is 0.
          final x2 = VietmapMathUtil.wrap(lng2 - lng1, -pi, pi);
          final x3Base = VietmapMathUtil.wrap(lng3 - lng1, -pi, pi);
          xTry[0] = x3Base;
          ...
          for (final x3 in xTry) {
            ...
            final t = len2 <= 0
                ? 0
                : VietmapMathUtil.clamp(
                    (x3! * x2 + (y3 - y1) * dy) / len2, 0, 1);
            final xClosest = t * x2;
            final yClosest = y1 + t * dy;
            final latClosest = VietmapMathUtil.inverseMercator(yClosest);
            final havDist =
                VietmapMathUtil.havDistance(lat3, latClosest, x3! - xClosest);
  ...
  static num _sinDeltaBearing(
    ...
    final b = sin(lat31) + 2 * sinLat1 * cosLat3 * VietmapMathUtil.hav(lng31);
    final d = sin(lat21) + 2 * sinLat1 * cosLat2 * VietmapMathUtil.hav(lng21);
  ...
  static bool _isOnSegmentGC(num lat1, num lng1, num lat2, num lng2, num lat3,
    ...
    final havDist13 = VietmapMathUtil.havDistance(lat1, lat3, lng1 - lng3);
    if (havDist13 <= havTolerance) {
      return true;
    }
    final havDist23 = VietmapMathUtil.havDistance(lat2, lat3, lng2 - lng3);
    if (havDist23 <= havTolerance) {
      return true;
    }
    final sinBearing = _sinDeltaBearing(lat1, lng1, lat2, lng2, lat3, lng3);
    final sinDist13 = VietmapMathUtil.sinFromHav(havDist13);
    final havCrossTrack = VietmapMathUtil.havFromSin(sinDist13 * sinBearing);
    if (havCrossTrack > havTolerance) {
      return false;
    }
    final havDist12 = VietmapMathUtil.havDistance(lat1, lat2, lng1 - lng2);
    ...
    final sinSumAlongTrack =
        VietmapMathUtil.sinSumFromHav(havAlongTrack13, havAlongTrack23);
  ...
  static num distanceToLine(
    ...
    final s0lat = VietmapMathUtil.toRadians(p.latitude);
    final s0lng = VietmapMathUtil.toRadians(p.longitude);
    final s1lat = VietmapMathUtil.toRadians(start.latitude);
    final s1lng = VietmapMathUtil.toRadians(start.longitude);
    final s2lat = VietmapMathUtil.toRadians(end.latitude);
    final s2lng = VietmapMathUtil.toRadians(end.longitude);
    ...
    if (u <= 0) {
      return VietmapSphericalUtil.computeDistanceBetween(p, start);
    }
    if (u >= 1) {
      return VietmapSphericalUtil.computeDistanceBetween(p, end);
    }
